...
class DeepQLearning:
    def __init__(self, game: SnakeGame, alpha=0.0001, gamma=0.9, epsilon_start=1.0, epsilon_end=0.1, epsilon_decay=0.995, tau=0.001):
        ...
        self.tau = tau
        self.policy_net = DQN(6, 3)  # 6 input dimensions (state) and 3 output dimensions (actions)
        self.target_net = DQN(6, 3)
    ...
    def learn(self):
        ...
        actions_tensor = torch.tensor([self.get_actions().index(action) for action in actions], dtype=torch.int64)
